...
export default class CouncilScreen extends React.Component {
    constructor(props) {
        super(props);
        this.state = {
            isloading: true,
    ...
    componentWillMount() {
        fetch('https://api.snowyguide.com/dev/pages/council', {
            method: 'GET',
            headers: {
            ...
            .then((res) => {
                if (res.message) {
                    ...
                    this.setState({ isloading: false, content: res.sources, introtext: res.introtext, images: res.images });
                }
            })
            .catch((err) => {
                this.setState({ isloading: false, news: [] })
    ...
    navigateToDetail(rowdata) {
        if (rowdata.url) {
            ...
            this.props.navigation.navigate("CouncilDetail", {council:rowdata});
            return;
        }
    }
    render() {
